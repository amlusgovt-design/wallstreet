...
export default function SignUp() {
	...
	const handleGoogleSignUp = async () => {
      ...
      const result = await toast.promise(signInWithPopup(auths, provider), {
        ...
        error: (err: any) => err?.message || "Error Occurred, Try Again",
      });
      ...
      if (!userDoc.exists()) {
        ...
        await Promise.all([
          setDoc(doc(db, "deposits", user.uid), { deposits: [] }),
          setDoc(doc(db, "withdrawals", user.uid), { withdrawals: [] }),
          setDoc(doc(db, "subscriptions", user.uid), { subscription: {} }),
          setDoc(doc(db, "verifications", user.uid), {
            verification: { document: null, status: "not-verified" },
          }),
          setDoc(doc(db, "accounts", user.uid), {
            account: { balance: 0, profit: 0, bonus: 0 },
          }),
          setDoc(doc(db, "trades", user.uid), { trades: [] }),
        ]);
      }
